...
# Le navigateur et le défi Cloudflare sont partagés avec l'amorceur Vinted
# (`collect/vinted_session.py`) : voir `collect/cloudflare.py`.
...
# appel réseau différé : le répit après `domcontentloaded` (`SETTLE_MS`, dans
# `cloudflare.py`) suffit à le laisser peupler. Mesuré à ~2 s.
...
class Browser:
    ...
    Le lancement et le défi eux-mêmes vivent dans `collect/cloudflare.py`,
    partagés avec l'amorceur Vinted.
    """
    ...
    def __init__(self, visible: bool = False, headless: bool = False):
        ...
        try:
            self._ctx = lancer_navigateur(self._play, data_dir() / "profil", visible, headless)
        except RuntimeError:
            self._play.stop()
            raise
        self._page = self._ctx.pages[0] if self._ctx.pages else self._ctx.new_page()
        self._defi = Defi(self._page, self._trace, auto=auto, visible=visible)

    @property
    def defis_leves(self) -> int:
        return self._defi.leves
    ...
    def _settle(self, url: str) -> bool:
        ...
        Une fois le `cf_clearance` obtenu, il vaut pour les pages suivantes."""
        return self._defi.franchir(url)
